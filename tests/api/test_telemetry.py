"""
POST /api/telemetry — Contract tests matching grader specification.
...
class TestTelemetryIngestion:
    ...
    def test_ingest_single_debris(self, session, reset_state):
        ...
        assert r.status_code == 200, (
            ...
            "FIX: DEBRIS objects must be accepted by /api/telemetry."
        )
    ...
    def test_ingest_mixed_objects(self, session, reset_state):
        """3 satellites + 5 debris in one payload → processed_count == 8."""
        objects = []
        for i in range(3):
        ...
        assert r.json().get("processed_count") == 8, (
            ...
            "FIX: Count both SATELLITE and DEBRIS objects in processed_count."
        )
    ...
    def test_response_schema(self, session, reset_state):
        ...
        assert body.get("status") == "ACK", (
            ...
            "FIX: Return {{'status': 'ACK', ...}} from POST /api/telemetry."
        )
        ...
        assert isinstance(body.get("active_cdm_warnings"), int), (
            ...
            "FIX: Include active_cdm_warnings in telemetry response."
        )

    def test_empty_objects_list(self, session, reset_state):
        """Empty objects list → 200 OK (or 422 if server requires non-empty)."""
    ...
    def test_invalid_type_rejected(self, session, reset_state):
        ...
        assert r.status_code == 422, (
            ...
            "FIX: Validate object type is one of SATELLITE, DEBRIS in telemetry schema."
        )

    def test_50_satellites_ingested(self, session, reset_state):
        """50 satellites in one payload → processed_count=50 and satellites_tracked=50."""
        objects = []
        for i in range(50):
            objects.append(state_to_obj(
                f"SAT-{i:02d}", "SATELLITE",
        ...
        assert status.get("satellites_tracked") == 50, (
            ...
            "FIX: Ensure all 50 objects stored in state after telemetry call."
        )
    ...
    def test_telemetry_upsert_semantics(self, session, reset_state):
        ...
        Sending 2 sats then 3 different sats → 5 total (not 3).
        """
        objects_2 = [
        ...
        assert tracked == 5, (
            ...
            "FIX: /api/telemetry must upsert (append new IDs, overwrite existing). "
    ...
    def test_iso_timestamp_format(self, session, reset_state):
        ...
        try:
            ...
            pytest.fail(
                ...
                "FIX: Format sim time as datetime.isoformat() + 'Z'."
            )
