"""
POST /api/maneuver/schedule â€” Contract tests matching grader specification.
"""
import math
import pytest
import sys
...
class TestManeuverSchedule:
    ...
    def test_schedule_valid_burn(self, session, reset_state):
        """Ingesting 1 sat and scheduling 1 burn â†’ HTTP 200/202, status='SCHEDULED'."""
        ...
        assert r.status_code in (200, 202), (
            ...
            "Requirement: Return 200 or 202 from POST /api/maneuver/schedule."
        )
        body = r.json()
        status_val = body.get("status", "")
        assert "SCHEDULED" in status_val.upper(), (
            f"Expected status containing 'SCHEDULED', got '{status_val}'. "
            "Requirement: ManeuverResponse must include status='SCHEDULED' on success."
        )
    ...
    def test_response_validation_fields(self, session, reset_state):
        ...
        assert "ground_station_los" in flat, (
            ...
            "Requirement: Add ground_station_los bool to ManeuverResponse."
        )
        ...
        assert "sufficient_fuel" in flat, (
            ...
            "Requirement: Add sufficient_fuel bool to ManeuverResponse."
        )
        ...
        assert "projected_mass_remaining_kg" in flat, (
            ...
            "Requirement: Compute and return projected post-burn mass."
        )
    ...
    def test_insufficient_fuel_flagged(self, session, reset_state):
        """Scheduling a burn that would consume more fuel than available â†’ sufficient_fuel=False."""
        state = circular_orbit_state(CONSTELLATION_ALT)
        post_telemetry(session, [state_to_obj("SAT-01", "SATELLITE", state)])
        ...
        if r.status_code in (200, 202):
            ...
            assert flat.get("sufficient_fuel") is False, (
                ...
                "Requirement: Compute total dv cost and compare against fuel_kg."
            )
        else:
    ...
    def test_unknown_satellite_rejected(self, session, reset_state):
        """Scheduling a burn for a satellite ID not in the system â†’ 404 or 400."""
        r = post_maneuver(session, "SAT-NONEXISTENT-XYZ", [
            _single_burn("BURN-001", "2026-03-12T08:01:00.000Z")
        ])
        assert r.status_code in (400, 404), (
            f"Expected 400 or 404 for unknown satellite, got {r.status_code}: {r.text}. "
            "Requirement: Lookup satellite by ID and return 404 if not found."
        )

    def test_burn_dv_magnitude_small(self, session, reset_state):
        """0.001 km/s (1 m/s) burn â†’ projected_mass_remaining_kg close to initial wet mass."""
        state = circular_orbit_state(CONSTELLATION_ALT)
        post_telemetry(session, [state_to_obj("SAT-01", "SATELLITE", state)])
        ...
        assert abs(projected - expected_remaining) < 1.0, (
            ...
            f"expected â‰ˆ{expected_remaining:.3f} kg (consumed {dm_expected:.4f} kg). "
            "Requirement: Apply Tsiolkovsky to compute projected remaining mass."
        )

    def test_burn_sequence_multiple(self, session, reset_state):
        """Schedule a 2-burn evasion+recovery sequence â†’ HTTP 200/202 with no error."""
        state = circular_orbit_state(CONSTELLATION_ALT)
        post_telemetry(session, [state_to_obj("SAT-01", "SATELLITE", state)])
        ...
        assert r.status_code in (200, 202), (
            ...
            "Requirement: POST /api/maneuver/schedule must accept multi-burn sequences."
        )
